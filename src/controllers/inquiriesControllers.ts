...
import InquiryModel, {IPopulatedUser} from "../models/inquiryModel";
...
const getInquiry = async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    let inquiry;
    try {
        inquiry = await InquiryModel.findById(inquiryId).populate<{ creator: IPopulatedUser }>("creator");
    } catch (err) {
