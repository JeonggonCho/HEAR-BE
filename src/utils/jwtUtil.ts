...
const jwtUtil = {
    ...
    refresh: async (userId: mongoose.Types.ObjectId) => {
        ...
        try {
            ...
            // 리프레쉬 토큰 저장
            const createdRefreshToken = await RefreshTokenModel.create([{userId, token}], {sess});
    ...
    refreshVerify: async (refreshToken: string, userId: string) => {
        ...
        try {
            existingRefreshToken = await RefreshTokenModel.findOne({userId, token: refreshToken});
